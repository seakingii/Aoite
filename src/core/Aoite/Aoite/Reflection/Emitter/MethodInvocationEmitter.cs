		...
		protected internal override Delegate CreateDelegate()
		{
			...
			if( CallInfo.HasRefParam )
			{
				...
				Generator.DeclareLocal( hasReturnType
				                        	? method.ReturnType
				                        	: Constants.ObjectType ); // T value;
				GenerateInvocation( method, paramArrayIndex, (byte) (startUsableLocalIndex + 1) );
				if( hasReturnType )
				{
					Generator.stloc( startUsableLocalIndex ); // value = <stack>;
				}
			...
			if( hasReturnType )
			{
				Generator.ldloc( startUsableLocalIndex ) // push value;
					.boxIfValueType( method.ReturnType ); // box value;
			}
			else
			{
