...
{
    internal class JObjectDeserializer
    {
        ...
        //private readonly static Regex DateTimeRegex = new Regex(@"^""\\/Date\((?<ticks>-?[0-9]+)(?:[a-zA-Z]|(?:\+|-)[0-9]{4})?\)\\/""", RegexOptions.Compiled);

        private const string DateTimeStart = "\"\\/Date(";
        private const string DateTimeEnd = ")\\/\"";
        private object DeserializeStringIntoDateTime()
        {
            ...
            var s = _s.Substring(pos + DateTimeSuffixLength);
            var moveCount = s.Length;
            //Match match = DateTimeRegex.Match(s);
            if(s.StartsWith(DateTimeStart))
            {
                s = s.Remove(0, DateTimeStart.Length);
                s = s.Substring(0, s.Length - DateTimeEnd.Length);
            ...
            string ticksStr = s;// match.Groups["ticks"].Value;

            long ticks;
            if(long.TryParse(ticksStr, out ticks))
            {
                _s.MoveNext(moveCount);

                // The javascript ticks start from 1/1/1970 but FX DateTime ticks start from 1/1/0001
                var dt = new DateTime(ticks * 10000 + JSerializer.DatetimeMinTimeTicks, DateTimeKind.Utc);
                return dt;
            }
            else {
