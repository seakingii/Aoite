...
namespace Aoite.Data
{
    ...
    public interface IDbExecutor
    {
        ...
        /// <returns>一个执行结果。</returns>
        DbResult<PageData<TEntity>> ToEntities<TEntity>(IPagination page);
        /// <summary>
        /// 执行分页查询命令，并返回自定义实体的集合结果。
        /// </summary>
        /// <typeparam name="TEntity">实体的类型。</typeparam>
        ...
        /// <returns>一个执行结果。</returns>
        DbResult<PageData<TEntity>> ToEntities<TEntity>(int pageNumber, int pageSize = 10);
        /// <summary>
        /// 执行查询命令，并返回自定义的实体结果。
        /// </summary>
        /// <typeparam name="TEntity">实体的类型。</typeparam>
        ...
        /// <param name="page">一个分页的实现。</param>
        /// <returns>一个执行结果。</returns>
        DbResult<PageData<dynamic>> ToEntities(IPagination page);
        /// <summary>
        /// 执行分页查询命令，并返回自定义实体的集合结果。
        /// </summary>
        /// <param name="pageNumber">以 1 起始的页码。</param>
        /// <param name="pageSize">分页大小。</param>
        /// <returns>一个执行结果。</returns>
        DbResult<PageData<dynamic>> ToEntities(int pageNumber, int pageSize = 10);
        /// <summary>
        /// 执行查询命令，并返回自定义的实体结果。
        /// </summary>
        /// <returns>一个执行结果。</returns>
